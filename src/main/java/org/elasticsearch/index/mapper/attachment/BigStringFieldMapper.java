 ...
 *
 */
public class BigStringFieldMapper extends AbstractFieldMapper<StringBuilder> implements AllFieldMapper.IncludeInAll {

    public static final String CONTENT_TYPE = "bigstring";
    ...
    public static class Builder extends AbstractFieldMapper.OpenBuilder<Builder, BigStringFieldMapper> {
        ...
        @Override
        public BigStringFieldMapper build(BuilderContext context) {
    ...
    public static class TypeParser implements Mapper.TypeParser {
        ...
        public Mapper.Builder parse(String name, Map<String, Object> node, ParserContext parserContext) throws MapperParsingException {
            BigStringFieldMapper.Builder builder = bigStringField(name);
    ...
    private StringBuilder nullValue;
    private StringBuilder value;
    private StringBuilder inner_reused_value = new StringBuilder();
    ...
    public FieldDataType defaultFieldDataType() {
        return new FieldDataType("bigstring");
    }
    ...
    protected Field parseCreateField(ParseContext context) throws IOException {
        ...
        if (context.externalValueSet()) {
            value = (StringBuilder) context.externalValue();
        } else {
                ...
                while ((token = parser.nextToken()) != XContentParser.Token.END_OBJECT) {
                    if (token == XContentParser.Token.FIELD_NAME) {
                        ...
                        if ("value".equals(currentFieldName) || "_value".equals(currentFieldName)) {
                        	...
                        	if (text != null) {
                        		inner_reused_value.setLength(0);
                        		inner_reused_value.append(text);
                        		value = inner_reused_value;
                        	} else {
                        		value = null;
							}
        ...
        if (context.includeInAll(includeInAll, this)) {
            context.allEntries().addText(names.fullName(), strValue, boost);
        }
        if (!fieldType().indexed() && !fieldType().stored()) {
            context.ignoredValue(names.indexName(), strValue);
            return null;
        }
        Field field = new StringField(names.indexName(), strValue, fieldType);
    ...
    public void merge(Mapper mergeWith, MergeContext mergeContext) throws MergeMappingException {
        ...
        if (!mergeContext.mergeFlags().simulate()) {
            this.includeInAll = ((BigStringFieldMapper) mergeWith).includeInAll;
            this.nullValue = ((BigStringFieldMapper) mergeWith).nullValue;
            this.ignoreAbove = ((BigStringFieldMapper) mergeWith).ignoreAbove;
        }
    }
